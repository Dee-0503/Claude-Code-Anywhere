...
describe('websocket HTTP upgrade authentication', () => {
  ...
  it('rejects missing and invalid tokens before handleUpgrade', async () => {
    ...
    await expect(
      rejectUpgrade(
        `ws://127.0.0.1:${port}/ws?device_id=${admin.device_id}&instance_id=${instance.id}&last_output_offset=0&last_input_offset=0`
      )
    ).resolves.toBe(401);
    await expect(
      rejectUpgrade(
        `ws://127.0.0.1:${port}/ws?device_id=${admin.device_id}&access_token=tampered-token&instance_id=${instance.id}&last_output_offset=0&last_input_offset=0`
      )
    ).resolves.toBe(401);
  ...
  it('rejects unauthorized, missing, and non-attachable instances before handleUpgrade', async () => {
    ...
    await expect(
      rejectUpgrade(
        `ws://127.0.0.1:${port}/ws?device_id=${member.device_id}&access_token=${member.access_token}&instance_id=${instance.id}&last_output_offset=0&last_input_offset=0`
      )
    ).resolves.toBe(403);
    await expect(
      rejectUpgrade(
        `ws://127.0.0.1:${port}/ws?device_id=${admin.device_id}&access_token=${admin.access_token}&instance_id=missing-instance&last_output_offset=0&last_input_offset=0`
      )
    ).resolves.toBe(404);
    ...
    await expect(
      rejectUpgrade(
        `ws://127.0.0.1:${port}/ws?device_id=${admin.device_id}&access_token=${admin.access_token}&instance_id=${instance.id}&last_output_offset=0&last_input_offset=0`
      )
    ).resolves.toBe(404);
    expect(bootstrapServer.registry.list()).toHaveLength(0);
  }, 10_000);
  ...
  it('rejects invalid origin and path and accepts authorized upgrades', async () => {
    ...
    await expect(
      rejectUpgrade(
        `ws://127.0.0.1:${port}/not-ws?device_id=${admin.device_id}&access_token=${admin.access_token}&instance_id=${instance.id}&last_output_offset=0&last_input_offset=0`
      )
    ).resolves.toBe(400);
