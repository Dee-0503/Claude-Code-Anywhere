...
function resolveAllowedCwd(cwd: string, allowedRoots: readonly string[]): string {
  ...
  try {
    ...
    throw serviceError("INVALID_MESSAGE", "Instance cwd must exist inside an allowed workspace root");
  }
...
export function createInstanceService(options: InstanceServiceOptions = {}) {
  ...
  const maxActiveInstancesPerDevice = options.maxActiveInstancesPerDevice ?? Number.POSITIVE_INFINITY;
  ...
  function activeInstances(): ClaudeInstance[] {
    return repository.list().filter((instance) => instance.status !== "exited" && instance.status !== "error");
  }
  ...
  function assertCapacity(deviceId: DeviceId): void {
    ...
    if (active.filter((instance) => instance.createdByDeviceId === deviceId).length >= maxActiveInstancesPerDevice) {
