...
function webSocketError(
  ...
  return createApiError(code as never, message, { statusCode }) as Error & {
    code: string;
    statusCode: number;
  };
}
...
export function validateWebSocketHandshake(
  ...
  if (!isNonNegativeInteger(params.last_output_offset)) {
    throw webSocketError(
      'INVALID_WEBSOCKET_HANDSHAKE',
      'last_output_offset must be a non-negative integer',
      400
    );
  }
  ...
  if (!isNonNegativeInteger(params.last_input_offset)) {
    throw webSocketError(
      'INVALID_WEBSOCKET_HANDSHAKE',
      'last_input_offset must be a non-negative integer',
      400
    );
  }
...
export async function authenticateWebSocketConnection(
  ...
  const deviceId =
    'device_id' in device && device.device_id !== undefined ? device.device_id : device.id;
  ...
  const attachableInstance = options.findAttachableInstanceForDevice(
    validParams.instance_id,
    deviceId
  );
  ...
  if (
    attachableInstance.status !== CLAUDE_INSTANCE_STATUSES.RUNNING &&
    attachableInstance.status !== CLAUDE_INSTANCE_STATUSES.IDLE
  ) {
    throw webSocketError(
      'WEBSOCKET_INSTANCE_NOT_ATTACHABLE',
      'WebSocket instance is not attachable',
      404
    );
  }
