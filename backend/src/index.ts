...
import {
  ...
  createSqliteInstanceRepository
} from './sessions/instance-repository.js';
...
export function createRuntimeServices(appConfig: AppConfig = config): RuntimeServices {
  const database =
    appConfig.repositoryMode === 'sqlite'
  ...
  const deviceRepository =
    database === null ? createInMemoryDeviceRepository() : createSqliteDeviceRepository(database);
  const pairingRepository =
    database === null ? createInMemoryPairingRepository() : createSqlitePairingRepository(database);
