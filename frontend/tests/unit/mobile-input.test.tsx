...
import { CommandTemplatePicker, renderCommandTemplatePicker } from "../../src/components/CommandTemplatePicker.js";
import { MobileShortcutBar, renderMobileShortcutBar } from "../../src/components/MobileShortcutBar.js";
...
describe("mobile input helpers", () => {
  ...
  it("sends a shortcut only after its button is clicked", async () => {
    ...
    await act(async () => {
      root.render(<MobileShortcutBar shortcuts={[{ label: "测试", input: "npm test\n" }]} onSend={(input) => sentInputs.push(input)} />);
    });
  ...
  it("requires an explicit paste confirmation before sending payload", async () => {
    ...
    await act(async () => {
      root.render(<PasteConfirm payload={"npm test\ngit status\n"} onConfirm={(payload) => confirmedPayloads.push(payload)} onCancel={() => { cancelled = true; }} />);
    });
