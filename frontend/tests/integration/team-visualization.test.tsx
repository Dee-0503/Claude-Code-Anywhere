...
describe('team visualization', () => {
  ...
  it('maps backend teammate instances to tabs and deep-link switching', () => {
    ...
    const routeModel = createTerminalRouteModel(
      instances,
      [
        {
          team_id: 'team-1',
    ...
    expect(routeModel.createTerminalPath('review-instance')).toBe(
      '/terminal?instance=review-instance'
    );
  ...
  it('fetches backend instance summaries with device credentials', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      expect(String(input)).toBe(
        'http://localhost:3000/api/instances?device_id=device-1&access_token=token-1'
      );
      return new Response(
        JSON.stringify({
          instances: [
  ...
  it('resolves an instance deep link from the URL', () => {
    expect(
      resolveInstanceIdFromLocation(new URL('https://example.test/terminal?instance=reviewer'))
    ).toBe('reviewer');
