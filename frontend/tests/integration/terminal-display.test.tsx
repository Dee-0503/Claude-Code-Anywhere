...
import {
  ...
  locateTerminalSearchMatch
} from '../../src/components/TerminalSearch.js';
...
} from '../../src/terminal/outputRenderer.js';
...
describe('terminal display', () => {
  ...
  it('keeps fallback output bounded without rebuilding the whole string on append', () => {
    const state = Array.from(
      { length: 80 },
      (_, index) => `${index.toString().padStart(2, '0')}-chunk\n`
    ).reduce(
      (current, data, index) =>
        appendTerminalOutput(
          current,
