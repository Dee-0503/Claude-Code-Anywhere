...
export function renderInstanceSwitcher(state: InstanceSwitcherState): string {
  ...
  const switchTargets = state.instances.filter(
    (instance) => instance.id !== state.currentInstanceId
  );
  const targetSummary =
...
export function InstanceSwitcher({
  ...
  instances,
  onSwitch
}: InstanceSwitcherProps): ReactElement {
  return (
      ...
      <ul>
        {instances.map((instance) => (
          <li key={instance.id}>
            <button
              type="button"
              disabled={instance.id === currentInstanceId}
