...
export function renderDeviceManager(devices: readonly DeviceManagerItem[]): string {
  ...
  const revocableSummary = revocable.length === 0
    ? "无可撤销成员设备"
...
export function DeviceManager({ devices, onRevoke, onTransferAdmin }: DeviceManagerProps): ReactElement {
  return (
        ...
        {devices.map((device) => (
          <li key={device.id}>
            ...
            {device.revokedAt === null && device.role !== "admin" ? (
              <>
                <button type="button" onClick={() => onRevoke?.(device.id)}>撤销</button>
                <button type="button" onClick={() => onTransferAdmin?.(device.id)}>转移管理员</button>
              </>
            ) : null}
          </li>
        ))}
